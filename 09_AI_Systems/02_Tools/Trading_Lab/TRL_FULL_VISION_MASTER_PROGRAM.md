...
| 6C | ALSAKKAF SCALPING Demo Automation V0 (TRL-R2-012) — informational row, not part of the 0–14 sequence; does not gate, reorder, or block Phases 7–14 | **Formally closed: committed and pushed, remotely verified.** `TRL_R2_012_ALSAKKAF_SCALPING_DEMO_AUTOMATION_V0_CONTRACT.md` authored (five product states, three strategy profiles, twenty-check demo-only preflight, R2-010 evidence bridge, hard risk caps, bounded Breakout Ladder with OCO, an independent adapter-level demo-account hard lock, a new separate hash-chained journal, an amendment granting `alsakkaf_scalping_demo_automation`/`market_intelligence_research` to the now-reachable `MT5_DEMO_AUTOMATED` mode). Implemented: `alsakkaf_scalping_data.py`/`_indicators.py`/`_strategy.py`/`_risk.py`/`_journal.py`/`_mt5.py`/`_service.py`/`_cli.py` (new, 8 modules), additive `mode_service.py`/`app.py`/`server.py`/`service.py`/dashboard (this checkpoint's first HTTP mutation routes, CSRF-token-gated), two launcher scripts, a committed synthetic fixture. 173 new tests across 11 modules; a 928-test/37-module targeted combined run; two consecutive clean full-suite runs (1249+173=1422, zero failures/errors — accepted pair reconstructed post-commit after a disclosed pre-existing intermittent flake in an untouched Phase 5 concurrency test); a real separate-process journal-lock proof; a full 27-step synthetic rehearsal against the fake adapter (Precision Scalping plan, Breakout Ladder with exact risk conservation and OCO, no lot escalation, order_check/order_send, restart reconciliation, daily-loss block, emergency stop with unrelated-order protection). A real 4T MT5 demo rehearsal was performed under explicit Founder authorization: all 20 preflight checks passed on the real terminal/demo account/XAUUSD, but the real market evaluation returned WAIT/NONE/0 — no `TRADE_CANDIDATE` existed, so `order_check`/`order_send` were correctly never reached. **Implementation commit `791ea98118539b6bf30f37e5eaf81ffd85d3589c` and consecutive-suite verification commit `0edd4adf2019e8162011ada58f3e5962555271eb` are both committed and pushed** (range `02c48e0..0edd4ad`, independently re-verified via a fresh `git fetch`). One operational validation item remains open (not a defect): an actual broker-verified `order_check`/`order_send` round trip, pending a genuine `TRADE_CANDIDATE` during a further, separately authorized bounded rehearsal. See `TRL_R2_012_ALSAKKAF_SCALPING_DEMO_AUTOMATION_V0_CONTRACT.md` and `_EVIDENCE.md` |
...
| 11 | Testing | Ongoing (complete-suite baseline 1422 tests, committed and pushed through TRL-R2-012 — 1249 pre-R2-012 + 173 net-new, passing twice consecutively with zero failures/errors) |
